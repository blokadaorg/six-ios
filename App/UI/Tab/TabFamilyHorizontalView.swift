...
struct TabFamilyHorizontalView: View {
    ...
    var content: some View {
        VStack {
            ...
            HStack(alignment: .bottom) {
                Spacer()
                TabItemView(
                    id: .Home, icon: "house", text: L10n.mainTabHome, badge: nil,
                    onTap: { self.onTap($0) }
                )
                TabItemView(
...
struct TabFamilyHorizontalView_Previews: PreviewProvider {
    static var previews: some View {
        ...
        return Group {
            TabFamilyHorizontalView(onTap: { _ in })
                .previewLayout(.sizeThatFits)
            TabFamilyHorizontalView(vm: packs, onTap: { _ in })
                .previewLayout(.sizeThatFits)
            TabFamilyHorizontalView(vm: account, onTap: { _ in })
                .previewLayout(.sizeThatFits)
                .environment(\.colorScheme, .dark)
            TabFamilyHorizontalView(vm: inbox, onTap: { _ in })
                .previewLayout(.sizeThatFits)
                .environment(\.colorScheme, .dark)
